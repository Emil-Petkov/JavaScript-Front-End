...
function getIdByGift(gift) {
    return fetch(BASE_URL)
        .then((res) => res.json())
        .then(
            (res) => Object.entries(res).find((e) => e[1].gift == gift)[1]._id
        );
}
...
function attachEventListeners() {
    ...
    changeButtons.forEach((changeButton) => {
        changeButton.addEventListener('click', (event) => {
            ...
            const price =
                taskElement.querySelector('p:nth-child(2)').textContent;
            const forE =
                taskElement.querySelector('p:nth-child(3)').textContent;
